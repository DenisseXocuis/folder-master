...
void menu(){
    system("clear||cls");
    puts("------------------------------------------------------");
    puts("-------CONVERSOR A DIVERSOS SISTEMAS NUMERICOS-------");
    puts("------------------------------------------------------");
...
void hex(int n){
    ...
    //Se ejecuta la función siempre y cuando dec(imal) no sea 0
    while (n != 0){
        //Se calcula el modulo de decimal respecto a 16 y se guarda temporalmente
        t=n%16;
            ...
            El valor puede ser entre 65 y 70, que corresponden a los ASCII para A a F*/
                ...
                t=t+55;
    //Guarda el resultado en un arreglo y divide dec(imal) entre 16 para continuar
        R[i]=t;
        n/=16;
        i++;
    }
...
for(int i=1;n != '\0'; i*=2){
    ...
    printf("El valor decimal es: %d", n1);
    putchar('\n');
}
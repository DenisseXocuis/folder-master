   ...
   /*__________LEER ARCHIVO__________*/
    ...
    {   FILE *archivo;
        ...
        {   BOOL = TRUE; //<- existe entrada de archivo
            ...
            size_t Bytes = fread(contenido,1,size,archivo);
            contenido[size+1] = '\0';
...
void borrar_lista(ADYACENCIA **lista)
{   ADYACENCIA *actuel = *lista;
    if(actuel)
    {
        *lista = (*lista)->next;
        free(actuel);
...
void borrar_grafo(GRAFO **grafo)
{   GRAFO *actuel = *grafo;
    if(actuel)
    {
        borrar_lista(&(*grafo)->adyacencia);
        *grafo = (*grafo)->next;
        free(actuel);
        if(!grafo) return;
        borrar_grafo(&*grafo);
...
int main()
{   GRAFO *def = NULL;
    BOOL = FALSE; int op, m=0; char name[50], frase[128], *respaldo;
    do
    {
        do
        {   menu(BOOL);

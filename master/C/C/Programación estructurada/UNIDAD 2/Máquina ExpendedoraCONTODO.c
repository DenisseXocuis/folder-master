...
void help();
void error(int);
void cam(float ,float); //cambio
void guardarA(struct datos matriz, int ); //función para guardar archivo

struct datos{ //datos de cada producto
    int clave,cantidad, tipo;
    char nombre[30];
    float precio;
};
...
int main(){
    int op,p=0,CLAVE,ac,cant,e=0,n,i,j,m=0,opc_arc; 
    float pago,din;
    struct compra *compras = NULL;
    int numCompras = 0;
    float totalRecaudadoRefrescos = 0.0;
    float totalRecaudadoBotanas = 0.0;
    ...
    /*                    GUÍA DE VARIABLES
            ...
            e = variable de control de las sentencias condicionales,
                determina si se encontró la información con éxito
            n = para el tamaño de matriz nxn
            i,j = variables de control
            m = variable de control para llenar matriz
            opc = variable para guardar o no el archivo
        */
    ...
    do{
        ...
        switch(op){
            ...
            case 5: /*CONSULTA DE TODOS LOS PRODUCTOS*/
                int guardar; //variable de eleccion para guardar todos los productos
                system("clear||cls");
                if(p<=0){ //si no hay productos registrados, imprime el error 3 (sin productos)
                        error(3);
                        break;
                ...
                if(p<=0){ //si no hay productos registrados, imprime el error 3 (sin productos)
                    ...
                    if(e!=0){
                        ...
                        if(pago<din) //si el dinero no es suficiente imprime error 4
                            ...
                            for(i = 0; i<n; i++){
                                for(j=0; j<n; j++){ //se posiciona de nuevo en el producto a comprar mediante la clave
                                if(matriz.ap[i][j].clave == CLAVE) 
                                    matriz.ap[i][j].cantidad -= cant; //resta la cantidad de productos q hay por la que se va a comprar
                                    break; //sale del bucle
                                }
                            }
                            cam(din,pago); //función de cambio 
                            }
                        }
                    }
            ...
            break;
            case 7: /*GUARDAR REGISTRO DE COMPRAS*/
                system("clear||cls");
                puts("----------Guardar registro de compras------------");
                puts("[1] Todas las compras");
                puts("[2] Compras de refrescos");
                puts("[3] Compras de botanas");
                printf("Escoga la opción deseada\n");
                scanf("%d", &opc_arc);
                if (opc_arc==1)
                    guardarRegistroCompras(compras, numCompras, "compras_todo.csv", &totalRecaudadoRefrescos, &totalRecaudadoBotanas);
                else if (opc_arc==2)
                    guardarRegistroCompras(compras, numCompras, "compras_refrescos.csv", &totalRecaudadoRefrescos, NULL);
                else if (opc_arc==3)
                    guardarRegistroCompras(compras, numCompras, "compras_botanas.csv", NULL, &totalRecaudadoBotanas);
                else
                    error(0);
                pause();
            break;
...
void guardarA(struct datos **ap, int n) {
    FILE *archivo;
    archivo = fopen("Productos.csv", "w"); //crea un archivo productos.csv
